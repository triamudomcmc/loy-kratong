...
const Filter = () => {
  ...
  const update = async (id: string, type: "archive" | "display") => {
        ...
        {list.map((item: any, i: number) => {
          return (<div className="flex justify-between space-x-4 shadow-md rounded-md w-[260px] px-4 py-2" key={`item-${i}`}>
            ...
            <div className="flex flex-col space-y-2 justify-center">
              <div onClick={() => {update(item.id, "display")}} className="w-6 h-6 flex justify-center items-center shadow-sm rounded-md border border-gray-200">
                <CheckIcon className="w-5 h-5 text-green-400"/>
              </div>
              <div onClick={() => {update(item.id, "archive")}} className="w-6 h-6 flex justify-center items-center shadow-sm rounded-md border border-gray-200">
